...
The script dumps out a CSV text file in which each line contains an entry.
  n00007846_64193.JPG,0.0060,0.2620,0.7545,0.9940

The entry can be read as:
  <JPG file name>, <xmin>, <ymin>, <xmax>, <ymax>

The bounding box for <JPG file name> contains two points (xmin, ymin) and
...
def ProcessXMLAnnotation(xml_file):
  ...
  for index in xrange(num_boxes):
    ...
    box.filename = GetItem('filename', root) + '.JPG'
    box.label = GetItem('name', root)
    print(f"{box.filename}, Box label: {box.label}")
